...
Outputs to docs/figures/fig1_*.png ... fig6_*.png. English labels (no CJK font).
Re-runnable: judges can regenerate by `python scripts/generate_report_figures.py`.
"""
...
def load_bag(name: str):
    """Load a bag (mean over seeds) — returns (oof_a, oof_p, oof_w)."""
    oa, op, ow = [], [], []
    for s in SEEDS:
        suf = "" if s == 42 else f"_seed{s}"
        d = np.load(CACHE / f"oof_test_{name}{suf}.npz")
        oa.append(d["oof_tt_a"]); op.append(d["oof_tt_p"]); ow.append(d["oof_tt_w"])
...
def fig1_v38_chain():
    ...
    # Move annotation out of the bar
...
# ============================================================
def fig2_flip_vs_lb():
    # From submission_forensics.py output
    data = [
    ...
    # Linear-fit trend line on log-ish trend (just for visual)
    z = np.polyfit(flips, deltas, 1)
    xs = np.linspace(flips.min() * 0.9, flips.max() * 1.05, 50)
    ax.plot(xs, np.polyval(z, xs), "--", color="gray", lw=1.2, label=f"linear fit: ΔLB ≈ {z[0]:.6f}·flips + {z[1]:.4f}")
...
def fig3_v25a_vs_v3_per_class():
    v3 = np.load(CACHE / "oof_test_probs.npz")
    v25a_oof_a, _, _ = load_bag("v25a")
    y_a = v3["oof_y_a"]
    base_a = v3["oof_lstm_a"]   # V3-LSTM as baseline reference
    base_pred = base_a.argmax(1)
    v25_pred = v25a_oof_a.argmax(1)

    # per-class F1
    f1_v3 = f1_score(y_a, base_pred, average=None, zero_division=0)
    f1_v25 = f1_score(y_a, v25_pred, average=None, zero_division=0)
    # Focus on first 15 effective action classes (15-18 are serve, model never predicts them)
    ...
    ax.bar(x - w / 2, f1_v3, w, color="#90A4AE", edgecolor="black", linewidth=0.5, label="V3-LSTM baseline")
    ax.bar(x + w / 2, f1_v25, w, color="#43A047", edgecolor="black", linewidth=0.5, label="V25-A (BiLSTM+SSL+opp-pair ctx)")
    ax.set_xticks(x)
...
def fig4_asym_spatial_label():
    # Synthetic label distribution for class 3 under V25-A FocalLoss vs V27 AsymSpatialFocalLoss
    N_POINT = 10
    LABEL_SMOOTH = 0.10
    ...
    # V25-A: uniform label smoothing 0.10
    v25 = np.full(N_POINT, LABEL_SMOOTH / (N_POINT - 1))
    v25[FOCUS_CLASS] = 1.0 - LABEL_SMOOTH

    # V27: class 3 80% self, 7.5% each at 2 & 6, 5%/6 spread to others
    ...
    for j in range(N_POINT):
        if j == FOCUS_CLASS:
            continue
        v27[j] = (FOCUS_SPATIAL_EPS / len(FOCUS_NEIGHBORS)) if j in FOCUS_NEIGHBORS else (FOCUS_UNIFORM_EPS / n_other)
    ...
    # Annotate spatial-neighbor classes
    for j in FOCUS_NEIGHBORS:
        ax.annotate("spatial\nneighbor", xy=(j + w / 2, v27[j]), xytext=(j + w / 2 + 0.4, v27[j] + 0.05),
...
def fig5_v38_failure_perclass():
    v3 = np.load(CACHE / "oof_test_probs.npz")
    y_a = v3["oof_y_a"]
    v27_oof_a, _, _ = load_bag("v27")
    v38_oof_a, _, _ = load_bag("v38")
    ...
    diff = f1_v38 - f1_v27   # positive = V38 better, negative = V38 worse
...
def fig6_consensus_inversion():
    # Data from cache/consensus_microflip/summary.csv (consensus_microflip_audit results)
    # head × k → OOF Final delta (negative everywhere)
    summary_path = CACHE / "consensus_microflip" / "summary.csv"
    if summary_path.exists():
        df = pd.read_csv(summary_path)
        # We want head/k matrix of nested_delta (or oof_delta)
        ...
        # Fallback: hardcoded from audit output
        ks = [2, 3, 4]
    ...
    ax.bar(x - w, a_vals, w, color="#1976D2", edgecolor="black", linewidth=0.5, label="action head")
    ax.bar(x,     p_vals, w, color="#FB8C00", edgecolor="black", linewidth=0.5, label="point head")
    ax.bar(x + w, b_vals, w, color="#6A1B9A", edgecolor="black", linewidth=0.5, label="both heads")
    ax.axhline(0, color="black", lw=0.6)
    ax.set_xticks(x)
    ax.set_xticklabels([f"k={k}\n({k} of 4 models agree)" for k in ks], fontsize=10)
    ax.set_xlabel("Consensus strictness — minimum # failed models that must agree", fontsize=11)
    ...
    ax.grid(True, alpha=0.25, axis="y")
    # Place annotation cleanly off to the side, point at k=4 group
...
def main():
    ...
    print(f"\nGenerated 6 figures into {FIG_DIR.relative_to(ROOT)}/")
